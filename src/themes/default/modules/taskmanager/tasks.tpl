<!-- BEGIN: main -->
<div class="taskmanager-tasks">
    <div class="page-header">
        <h1>{LANG.task_list}</h1>
    </div>
    
    <div class="table-responsive">
        <table class="table table-striped table-hover">
            <thead>
                <tr>
                    <th>{LANG.task_title}</th>
                    <th>{LANG.task_project}</th>
                    <th>{LANG.task_assigned_to}</th>
                    <th>{LANG.task_status}</th>
                    <th>{LANG.task_deadline}</th>
                    <th>{LANG.task_progress}</th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                <!-- BEGIN: task -->
                <tr>
                    <td><a href="{TASK.link}">{TASK.title}</a></td>
                    <td>{TASK.project_title}</td>
                    <td>{TASK.assigned_username}</td>
                    <td><span class="badge" style="background-color: {TASK.status_color}">{TASK.status_name}</span></td>
                    <td>{TASK.deadline_format}</td>
                    <td>
                        <div class="progress" style="height: 20px;">
                            <div class="progress-bar" role="progressbar" style="width: {TASK.progress}%">
                                {TASK.progress}%
                            </div>
                        </div>
                    </td>
                    <td>
                        <a href="{TASK.link}" class="btn btn-sm btn-info">{LANG.task_detail}</a>
                    </td>
                </tr>
                <!-- END: task -->
            </tbody>
        </table>
        
        <!-- BEGIN: empty -->
        <div class="alert alert-info">
            {LANG.no_data}
        </div>
        <!-- END: empty -->
    </div>
    
    <!-- BEGIN: generate_page -->
    <div class="text-center mt-4">
        {GENERATE_PAGE}
    </div>
    <!-- END: generate_page -->
</div>
<!-- END: main -->
